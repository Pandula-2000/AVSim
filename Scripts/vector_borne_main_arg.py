...
controlled = check_bool(args[2])

if controlled:
    VEC_CONTROL_RATE = float(args[3])
    MAX_EXP_AGENTS = int(args[4])
else:
    VEC_CONTROL_RATE = None
    MAX_EXP_AGENTS = None

temp_min = int(args[5])
temp_max = int(args[6])

plot_results = check_bool(args[7])
...
if __name__ == "__main__":
    ...
    # ------------------------------------------- SET SIMULATION NAME ------------------------------------------------------
    simName = "VB_Simulation"

    folder = f'../Results/{simName}_plots({plot_results})_Controlled_({controlled}-{VEC_CONTROL_RATE}_max-{MAX_EXP_AGENTS})_temp_({temp_min},{temp_max})_{simDays}_Days_{current_date}_{current_time}'

    print(VECTOR_CONTROLLING_RATE, MAX_EXPOSED_AGENTS)
    print(folder)
    ...
    if plot_results:
        ...
        person_disease_state_printer = None
        time_table_writer = None
    
    # ------------------------------------------- INITIALIZE THE SIMULATION ------------------------------------------------
    env = LaunchEnvironment()
    # Initialize patch grids
    PATCHES = init_patch_grid(env, temp_min, temp_max)
    ...
    # assign_initial_infection(env, "AgriculturalZone", 2, "any", 1, ((50,45), (45,20), (20,10)), 3)
    # assign_initial_infection(env, "ResidentialZone_5", 10, "any")
    # assign_initial_infection(env, specific="Home_1")
    assign_initial_infection(env, "ResidentialZone_5", 1, "Home", 6, ((50,45), (60,40), (55,35)), 5)
    ...
    for DAY in range(1, simDays+1):
        ...
        if(DAY == 1 or DAY % 10 == 0 and plot_results):
