...
# ------------------------------------------- SET SIMULATION NAME ------------------------------------------------------
# simName = "Quarantine_NVaccine_TEST_1mRadius_test1"

# ------------------------------------------- SET MANUAL PARAMETERS ----------------------------------------------------
infect_random = check_bool(args[1])
simDays = int(args[2])
quarantine_agents = check_bool(args[3])

quarantine_by_PCR = check_bool(args[8])
quarantine_by_class = check_bool(args[9])

run_vaccine = check_bool(args[4])

pandemic_detection_threshold = float(args[7])

percentage_of_agents_to_infect = 0.002
classes_to_infect = ['Student']

step_disease_frequency = 5

location_max_risk = float(args[5])
bus_max_risk = float(args[6])

radius_of_infection = 1

simName = f"Random_{args[1]}Q_{args[3]}_V_{args[4]}"
...
simName += f"_Qpcr_{args[8]}_Qclass_{args[9]}"
...
# ------------------------------------------------ SET PATHS -----------------------------------------------------------
save_folder_name = f"AutomatedTest_Quarantine"
save_path = Path(f"../Results/{save_folder_name}/{simName}_{current_date}_{current_time}")
...
plt.title(f'Air Borne Disease spread over Time for Quarantine = {args[2]} and Vaccination = {args[3]}')
